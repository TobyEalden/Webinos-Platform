...
!define VERSION "v0.5.1-Eng-Eval"
...
;Modern UI Configuration
  ...
  !define MUI_FINISHPAGE_RUN "$INSTDIR\bin\webinosMonitor.exe"
...
;  File /r "${SRCROOT}\storage\*.*"
  ...
  File "${RedistPath}\node.exe"
  File "${RedistPath}\webinosNodeService.exe"
  File "${RedistPath}\webinosMonitor.exe"
	...
	!insertmacro WriteRegStringIfUndef HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Run" "${PRODUCT_NAME}Monitor"  "$INSTDIR\bin\webinosMonitor.exe"

	; Service (auto starting)
	nsSCM::Install /NOUNLOAD "webinos_pzh" "webinos pzh" 16 2 "$INSTDIR\bin\webinosNodeService.exe" "" "" "" ""
	; Pop $0 ; return error/success

	CreateDirectory $APPDATA\webinos
	FileOpen $0 $APPDATA\webinos\webinos_pzh.dat w
	FileWrite $0 "nodePath|$INSTDIR\bin|workingDirectoryPath|$INSTDIR|nodeArgs|webinos_pzh.js|instance|0|"
	FileClose $0
	...
	; Service (auto starting)
	nsSCM::Install /NOUNLOAD "webinos_pzp" "webinos pzp" 16 2 "$INSTDIR\bin\webinosNodeService.exe" "" "webinos_pzh" "" ""

	CreateDirectory $APPDATA\webinos
	FileOpen $0 $APPDATA\webinos\webinos_pzp.dat w
	FileWrite $0 "nodePath|$INSTDIR\bin|workingDirectoryPath|$INSTDIR|nodeArgs|-|instance|0|"
	FileClose $0
...
addClientStartShortcut:
  ...
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" $INSTDIR\bin\webinosMonitor.exe "$INSTDIR\${PRODUCT_ICON}"
