...
module.exports = function (app, address, port) {
    ...
    app.get('/main/:useremail/request-access-authenticate', function (req, res) {
        ...
        externalRelyingParty.authenticate(identifierUrl, false, function (error, authUrl) {
            if (error) {
                res.render("problem",{ id:"problem", appTitle: appTitle, title: "Problem", error: 'Authentication failed: ' + error.message});
            } else if (!authUrl) {
                res.render("problem",{ id:"problem", appTitle: appTitle, title: "Problem", error: 'Authentication failed, no authorisation url.'});
            } else {
        ...
        externalRelyingParty.verifyAssertion(req, function (error, result) {
            if (error) {
                ...
                if (!req.session.expectedExternalAuth.externalCertUrl) {
                    res.render("problem",{ id:"problem", appTitle: appTitle, title: "Problem", error: 'Failed to read cookies.'});
                }
                    ...
                    helper.getCertsFromHostDirect(options, function (certs) {
                        ...
                        pzhAdaptor.requestAddFriend(session.internalUser, result, pzhData, function (status) {
                            ...
                            if (status.message) {
                                ...
                                res.render("problem",{ id:"problem", appTitle: appTitle, title: "Problem", error: 'User ' + result.email + ' different from the one that initiated request'});
                            }
                        });
                    }, function (err) {
                        res.render("problem",{ id:"problem", appTitle: appTitle, title: "Problem", error: 'Failed to retrieve certificate from remote host'});
                    });
                }
            }
        });
