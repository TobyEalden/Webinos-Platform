...
SectionIn RO
  ...
  File "${SRCROOT}\webinos_branding.json"
  File "${SRCROOT}\ubiapps_branding.json"
  File "${SRCROOT}\vodafone_branding.json"
...
; They are also required in the dcrypt folder (some reason they are not loaded using PATH)	
  ...
  File "${RedistPath}\node.exe"
	...
	FileWrite $0 "{$\"nodePath$\": $\"$1$\",$\"workingDirectoryPath$\": $\"$2$\",$\"pzp_nodeArgs$\": $\"webinos_pzp.js --widgetServer --branding=ubiapps$\",$\"pzh_nodeArgs$\": $\"webinos_pzh.js$\"}"
	FileClose $0
